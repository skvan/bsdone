...
import org.springframework.boot.loader.net.protocol.jar.Canonicalizer;
import org.springframework.boot.loader.net.protocol.jar.JarUrlConnection;
...
extends URLStreamHandler {
    ...
    protected void parseURL(URL url, String spec, int start, int limit) {
        ...
        this.setURL(url, PROTOCOL, "", -1, null, null, path, null, ref);
    }
    ...
    private String extractAbsolutePath(String spec, int start, int limit) {
        int indexOfSeparator = Handler.indexOfSeparator(spec, start, limit);
    ...
    private String extractRelativePath(URL url, String spec, int start, int limit) {
        ...
        return Canonicalizer.canonicalizeAfter(path, Handler.indexOfSeparator(path) + 1);
    }

    private String extractContextPath(URL url, String spec, int start) {
        String contextPath = url.getPath();
        if (spec.regionMatches(false, start, "/", 0, 1)) {
            int indexOfContextPathSeparator = Handler.indexOfSeparator(contextPath);
    ...
    private void assertInnerUrlIsNotMalformed(String spec, String innerUrl) {
        ...
        try {
            ...
            throw new IllegalStateException("invalid url: %s (%s)".formatted(new Object[]{spec, ex}));
        }
    }
    ...
    protected boolean sameFile(URL url1, URL url2) {
        ...
        if (!entry1.equals(entry2 = file2.substring(indexOfSeparator2 + 2))) {
            return false;
        }
        try {
    ...
    static int indexOfSeparator(String spec) {
        return Handler.indexOfSeparator(spec, 0, spec.length());
    }

    static int indexOfSeparator(String spec, int start, int limit) {
        for (int i = limit - 1; i >= start; --i) {
            if (spec.charAt(i) != '!' || i + 1 >= limit || spec.charAt(i + 1) != '/') continue;
            return i;
        }
        return -1;
    }
