...
import com.bsball.service.iplocation.ChinaCoordTransform;
 ...
 */
@Component
public class BaiduIpLocationProvider
implements IpLocationProvider {
    @Generated
    private static final Logger log = LoggerFactory.getLogger(BaiduIpLocationProvider.class);
    private static final RestClient REST = RestClient.builder().requestFactory((ClientHttpRequestFactory)new JdkClientHttpRequestFactory(HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(10L)).build())).build();
    ...
    public boolean isConfigured() {
        OpenPlatformProperties.Provider p = (OpenPlatformProperties.Provider)this.props.getLbs().getProviders().get("baidu");
        return p != null && p.isEnabled() && p.getKey() != null && !p.getKey().isBlank();
    }

    public LbsProvider lbsProvider() {
        return LbsProvider.BAIDU;
    }
    ...
    public Optional<IpLocationDetail> locate(String ip) {
        ...
        String endpoint = api.getEndpoint() != null && !api.getEndpoint().isBlank() ? api.getEndpoint() : "https://api.map.baidu.com/location/ip";
        ...
        try {
            ...
            if (st != 0) {
                if (BaiduIpLocationProvider.isQosLike((String)root.path("message").asText(""))) {
                    this.qosBlockedUntilMs = System.currentTimeMillis() + 300000L;
                    log.warn("\u767e\u5ea6 IP \u5b9a\u4f4d\u89e6\u53d1\u9650\u6d41/\u914d\u989d\uff0c\u8fdb\u5165\u51b7\u5374 {} \u79d2", (Object)300L);
                }
                Optional<IpLocationDetail> optional = Optional.empty();
                return optional;
            }
            JsonNode detail = root.path("content").path("address_detail");
            String prov = detail.path("province").asText("");
            String city = detail.path("city").asText("");
            String adcode = detail.path("adcode").asText("");
            if (adcode.isBlank()) {
                adcode = detail.path("city_code").asText("");
            }
            if ((formatted = IpRegionTextFormatter.format((String)prov, (String)city)).isEmpty()) {
                Optional<IpLocationDetail> optional = Optional.empty();
                return optional;
            }
            Point gcjPt = BaiduIpLocationProvider.parseBaiduPointBd09ToGcj((JsonNode)root);
            Optional<IpLocationDetail> optional = Optional.of(new IpLocationDetail(formatted, BaiduIpLocationProvider.blankToNull((String)prov), BaiduIpLocationProvider.blankToNull((String)city), BaiduIpLocationProvider.blankToNull((String)adcode), null, gcjPt, LbsProvider.BAIDU));
            return optional;
        }
        catch (Exception e) {
            log.debug("\u767e\u5ea6 IP \u5b9a\u4f4d\u5f02\u5e38: {}", (Object)e.getMessage());
            Optional<IpLocationDetail> optional = Optional.empty();
            return optional;
        }
        finally {
    ...
    private static Point parseBaiduPointBd09ToGcj(JsonNode root) {
        JsonNode pointNode = root.path("content").path("point");
        if (pointNode.isMissingNode() || !pointNode.isObject()) {
            return null;
        }
        String xs = pointNode.path("x").asText("");
        String ys = pointNode.path("y").asText("");
        if (xs.isBlank() || ys.isBlank()) {
            return null;
        }
        try {
            double bdLng = Double.parseDouble(xs.trim());
            double bdLat = Double.parseDouble(ys.trim());
            double[] gcj = ChinaCoordTransform.bd09ToGcj02((double)bdLng, (double)bdLat);
            return IpGcj02.point((double)gcj[0], (double)gcj[1]);
        }
    ...
    private TokenBucketRateLimiter getRateLimiter() {
        if (this.rateLimiter == null) {
            BaiduIpLocationProvider baiduIpLocationProvider = this;
            synchronized (baiduIpLocationProvider) {
                if (this.rateLimiter == null) {
                    OpenPlatformProperties.Provider p = (OpenPlatformProperties.Provider)this.props.getLbs().getProviders().get("baidu");
    ...
    @Generated
    public BaiduIpLocationProvider(OpenPlatformProperties props, ObjectMapper objectMapper) {
        this.props = props;
